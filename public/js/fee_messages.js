...
function fillMessages() {
    if ($('#hidden_page').val() == "W")
        fillWithdrawMessages()
    else if ($('#hidden_page').val() == "D")
        fillDepositMessages()
    else if ($('#hidden_page').val() == "DCS")
        fillDepositSendMessages()
...
function fillWithdrawMessages(){
    ...
    if($('#banks').val() == "0" || $('#banks').val() == country_settings.preferential_bank1_code || $('#banks').val() == country_settings.preferential_bank2_code || $('#banks').val() == country_settings.preferential_bank3_code || $('#banks').val() == country_settings.preferential_bank4_code) {
        if($.isNumeric($('#value').val())) {
            $('#calc_withdraw_fee').html((parseFloat($('#value').val()) * country_settings.fee_withdrawal_percent/100 + parseFloat(country_settings.nominal_fee_withdrawal_preferential_bank)).toFixed(2));
        } else {
            $('#calc_withdraw_fee').html(" - - - ");
        }
        calc_value = ($('#hidden_fees_information').attr('wallet_available') + $('#hidden_fees_information').attr('wallet_crypto') - parseFloat(country_settings.nominal_fee_withdrawal_preferential_bank).toFixed(2)) * (1 - country_settings.fee_withdrawal_percent/100);
        ...
        if($.isNumeric($('#value').val())) {
            $('#calc_withdraw_fee').html((parseFloat($('#value').val()) * country_settings.fee_withdrawal_percent/100 + parseFloat(country_settings.nominal_fee_withdrawal_not_preferential_bank)).toFixed(2));
        } else {
            $('#calc_withdraw_fee').html(" - - - ");
        }
        calc_value = ($('#hidden_fees_information').attr('wallet_available') + $('#hidden_fees_information').attr('wallet_crypto') - parseFloat(country_settings.nominal_fee_withdrawal_not_preferential_bank).toFixed(2)) * (1 - country_settings.fee_withdrawal_percent/100);
        if (calc_value < country_settings.minimum_value ) { calc_value -= country_settings.minimum_value * 0.02 };
        if (calc_value < 0 ) calc_value = 0;
        $('#calc_maximum_withdraw').html(calc_value.toFixed(2));
    }
...
function fillDepositMessages() {
    if (country_settings.fee_deposit_percent != 0 && $.isNumeric($('#value').val()))
        $('#calc_deposit_fee').html((parseFloat($('#value').val()) * country_settings.fee_deposit_percent/100).toFixed(2))
    else
...
/* Fills the spaces for fees and values information inside Deposit-Send page */
function fillDepositSendMessages() {
/*    if (country_settings.fee_deposit_percent != 0 && $.isNumeric($('#value').val()))
        $('#calc_deposit_fee').html((parseFloat($('#value').val()) * country_settings.fee_deposit_percent/100).toFixed(2))
    else
        $('#calc_deposit_fee').html("- - - ");
...
function fillSendMessages() {
    if (country_settings.fee_send_percent != 0 && $.isNumeric($('#value').val()))
        $('#calc_send_fee').html((parseFloat($('#value').val()) * country_settings.fee_send_percent/100).toFixed(2))
    else
        $('#calc_send_fee').html("- - - ");
    $('#calc_maximum_send').html((($('#hidden_fees_information').attr('wallet_available') + $('#hidden_fees_information').attr('wallet_crypto')) * (1 - country_settings.fee_send_percent/100)).toFixed(2));
}
...
function fillToFiatMessages() {
    ...
    if (country_settings.fee_tofiat_percent != 0 && $.isNumeric($('#value').val()))
        $('#calc_convertion_net').html((parseFloat($('#value').val()) * (100 - country_settings.fee_tofiat_percent)/100).toFixed(2))
    else
        $('#calc_convertion_net').html("- - - ");
    $('#calc_convertion_rate').html((parseFloat(100 - country_settings.fee_tofiat_percent)/100).toFixed(3))
